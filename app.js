...
async function handleLogin() {
    ...
    try {
        ...
        if (doc.exists && doc.data().password === passVal) {
            ...
            currentUser.username = doc.id; // WICHTIG: Die ID ist der Username
            ...
            if(document.getElementById('current-rank')) 
            ...
            // --- LIVE SYSTEME STARTEN ---
            startWantedListener(); // Gesuchte Personen
            initDashboard();       // BOLO Feed
            initDispatchMonitor(); // Leitstelle
            updateMyStatus('10-8');// Auto-Login Status
...
function applyTheme(dept) {
    ...
    if (dept === "MARSHAL") {
        ...
        if(header) {
            ...
            header.classList.remove("text-blue-400", "text-purple-500");
        ...
        if(header) {
            ...
            header.classList.remove("text-blue-400", "text-amber-500");
        ...
        if(header) {
            ...
            header.classList.remove("text-amber-500", "text-purple-500");
...
async function savePerson() {
    ...
    // FIX: Vorname + Nachname als Suchbegriff
...
// ==========================================
// 8. EMPLOYEES & LAWS (STGB RECHNER)
// ==========================================

// HIER FÜGST DU DEINE 350 GESETZE EIN:
const LAWS = [
    // Beispiel Zeilen (Lösche diese und füge deine Liste ein):
...
function loadLaws(searchTerm = "") {
    ...
    // Suche filtern (Groß/Kleinschreibung egal)
    const term = searchTerm.toLowerCase();
    
    // Wir filtern die Liste
    const filtered = LAWS.filter(l => 
        l.id.toLowerCase().includes(term) || 
        l.de.toLowerCase().includes(term) ||
        (l.text && l.text.toLowerCase().includes(term))
    );

    if(filtered.length === 0) {
        list.innerHTML = "<div class='text-slate-500 text-xs p-2'>Kein Gesetz gefunden.</div>";
        return;
    }

    // Performance: Bei leerer Suche zeigen wir nur die ersten 50, damit es nicht laggt
    const displayList = (term === "") ? filtered.slice(0, 50) : filtered;

    displayList.forEach(l => {
        // HTML für jeden Eintrag
        list.innerHTML += `
            <div class="p-3 hover:bg-slate-700 cursor-pointer border-b border-slate-700/50 group" onclick="addToCart('${l.id}')">
                <div class="flex justify-between items-center mb-1">
                    <span class="font-bold text-blue-400 text-xs bg-blue-900/20 px-1 rounded border border-blue-900">${l.id}</span>
                    <span class="text-green-400 font-mono font-bold">$${l.fine}</span>
                </div>
                <div class="font-bold text-sm text-slate-200">${l.de}</div>
                <div class="text-[10px] text-slate-500 truncate group-hover:whitespace-normal group-hover:text-slate-300 transition-colors">
                    ${l.text || l.en}
                </div>
                ${l.jail > 0 ? `<div class="text-[10px] text-red-500 mt-1 font-bold">Haftzeit: ${l.jail} Monate</div>` : ''}
            </div>
        `;
    });
    
    if(term === "" && LAWS.length > 50) {
        list.innerHTML += "<div class='text-center text-[10px] text-slate-600 p-2 italic'>... tippe um mehr zu suchen ...</div>";
    }
}

function addToCart(id) {
    const item = LAWS.find(l => l.id === id);
    if(item) {
        cart.push(item);
        renderCart();
    }
...
function renderCart() {
    ...
    if (cart.length === 0) {
        cartDiv.innerHTML = "<p class='text-slate-500 text-center italic mt-10 text-xs'>Klicke links auf Gesetze</p>";
        updateTotal();
        return;
    }
    ...
    cart.map((c, i) => {
        ...
        <div class="flex justify-between items-center text-xs p-2 border-b border-slate-700 bg-slate-800/30 mb-1 rounded">
            <div class="flex flex-col">
                <span class="font-bold text-slate-300">${c.de}</span>
                <span class="text-[10px] text-slate-500">${c.id}</span>
            </div>
            <div class="flex items-center gap-3">
                <span class="text-green-500">$${c.fine}</span>
                ${c.jail > 0 ? `<span class="text-red-500 border border-red-900 px-1 rounded">${c.jail} HE</span>` : ''}
                <button onclick="cart.splice(${i},1);renderCart()" class="text-slate-500 hover:text-red-500 font-bold px-2">✕</button>
            </div>
        </div>`;
    });
    updateTotal();
...
function updateTotal() {
    // Hier nutzen wir jetzt 'fine' und 'jail' statt 'price'
...
// EMPLOYEES (Personalverwaltung bleibt gleich)
async function renderEmployeePanel() {
    const list = document.getElementById('employee-list');
    if(!list) return;
    const snap = await db.collection('users').get();
                    ...
                    <div class="glass-panel p-3 rounded border-l-4 ${borderClass} ${bgClass} flex gap-3 relative animate-fadeIn">
                        ...
                        <div class="flex-1">
                            <div class="flex justify-between items-start">
                                ...
                                <div class="flex items-center"><span class="text-[10px] font-mono text-slate-400 mr-2">${time} Uhr</span>${deleteBtn}</div>
                            </div>
...
async function deleteBOLO(id) {
    if(confirm("Löschen?")) await db.collection('bolos').doc(id).delete();
}
...
async function updateMyStatus(newStatus) {
    ...
    try {
        // Benutze .set mit merge: true, um Fehler zu vermeiden
...
function initDispatchMonitor() {
    ...
    dispatchUnsubscribe = db.collection('users').onSnapshot(snapshot => {
        ...
        snapshot.forEach(doc => {
            const u = doc.data();
            const unitName = doc.id; // Name aus ID nehmen!
